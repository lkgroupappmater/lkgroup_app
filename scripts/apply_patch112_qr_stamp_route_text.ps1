...
foreach($f in $files){ if(!(Test-Path -LiteralPath $f)){ throw "Missing file: $f" } }

function Patch-Form($f, $isStatement) {
  $x = Get-Content -LiteralPath $f -Raw -Encoding UTF8

  # Inland delivery body must be blank when there is no linked delivery data.
  $x = [regex]::Replace(
    $x,
  ...
  # QR/account area: use full document width, four equal panels, minimal gaps.
  # Keep original QR aspect ratio through existing _imageContain().
  $pattern = "(?s)    final payTop = sumTop \+ 204;.*?    final noteTop = payTop \+ 150;"
  $replacement = @'
...
'@
  if([regex]::IsMatch($x,$pattern,[Text.RegularExpressions.RegexOptions]::Singleline)){
    $x=[regex]::Replace($x,$pattern,$replacement,1,[Text.RegularExpressions.RegexOptions]::Singleline)
  } else {
    Write-Host "NOTE: payment block not matched in $f" -ForegroundColor Yellow
  }

  # Make QR larger in each quarter while preserving ratio.
  $x = $x.Replace(
  ...
  # Signature boxes narrower; company stamp centered in the blank part of LK Trading box.
  ...
  # Route-specific footer/remark: use route data instead of hard-coded generic lines.
  if($isStatement){
  ...
  # Replace center signature notice only; Remark block already calls RouteCatalog.remarkFor where available.
  $noticePattern = "(?s)    _text\(c,\s*'\* 운임은 USD 기준입니다\..*?14, center: true\);"
  $noticeReplacement = @"
...
"@
  $x=[regex]::Replace($x,$noticePattern,$noticeReplacement,1,[Text.RegularExpressions.RegexOptions]::Singleline)
...
Write-Host ""
Write-Host "Patch112 applied." -ForegroundColor Green
Write-Host "- four full-width payment/account panels"
Write-Host "- QR aspect ratio preserved"
Write-Host "- LK stamp centered"
Write-Host "- Inland delivery blank when empty"
Write-Host "- route remark used for lower center notice"
