-- Patch199B1
-- 1) resolved unknown cargo: release XX before normal resequence
-- 2) batch Remark exact-fragment dedupe after finalizer
-- Safe-update policy: every UPDATE has explicit WHERE.
...
immutable
as $$
  select coalesce(string_agg(token, ' / ' order by first_ord), '')
  from (
    select min(ord) as first_ord, btrim(token) as token
    from unnest(regexp_split_to_array(coalesce(p_text,''), E'\\s*/\\s*'))
      with ordinality as t(token, ord)
    where btrim(token) <> ''
    group by lower(regexp_replace(btrim(token), E'\\s+', ' ', 'g'))
  ) q;
$$;
...
-- Existing table/RPC kept for compatibility, but from Patch199B onward this row
-- means MANUAL ADDITIONAL discount, not replacement discount.
