...
class _CargoManagementScreenState extends State<CargoManagementScreen> {
  ...
  void initState() {
    ...
    for (final raw in widget.initialShipments) {
      ...
      if (id != null && _shipments.every((item) => item['id'] != id)) _shipments.add(raw.map((key, value) => MapEntry(key, value.toString())));
      if (id != null) _selectedIds.add(id);
    }
    if (_selectedIds.isNotEmpty) {
      _selectedId = _selectedIds.first;
      final item = _shipments.firstWhere((row) => row['id'] == _selectedId);
  ...
  @override
  void dispose() {
    _invoiceController.dispose(); _nameController.dispose(); _phoneController.dispose(); _noteController.dispose(); super.dispose();
  }
  ...
  List<Map<String, String>> get _results {
    ...
    return _shipments.where((item) {
      ...
      final yearMatches = _year == '전체' || item['year'] == _year;
      final voyageMatches = _voyage == '전체' || item['voyage'] == _voyage;
      ...
      return routeMatches && yearMatches && voyageMatches && invoiceMatches && nameMatches && phoneMatches;
    }).toList();
  }
  ...
  void _save() {
    if (_invoiceController.text.trim().isEmpty || _nameController.text.trim().isEmpty) { _message('송장번호와 수령인 이름을 입력해 주세요.'); return; }
    final changes = <String, String>{'invoice': _invoiceController.text.trim(), 'name': _nameController.text.trim(), 'phone': _phoneController.text.trim(), 'route': _route, 'year': _year, 'voyage': _voyage};
    if (_canSaveDirectly) {
  ...
  void _cancelSelection() => setState(() { _selectedIds.clear(); _selectedId = null; _invoiceController.clear(); _nameController.clear(); _phoneController.clear(); _noteController.clear(); });
  void _message(String message) { if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(message), backgroundColor: AppColors.primary)); }
  ...
  Future<void> _openManagementMenu() async {
    final choice = await showModalBottomSheet<String>(context: context, builder: (sheetContext) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [ListTile(leading: const Icon(Icons.calendar_month), title: const Text('선적 일정 관리'), onTap: () => Navigator.pop(sheetContext, 'schedule')), ListTile(leading: const Icon(Icons.campaign_outlined), title: const Text('공지사항 관리'), onTap: () => Navigator.pop(sheetContext, 'notice'))])));
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      DropdownButtonFormField<String>(value: _route, decoration: _decoration('운송 경로', Icons.route), items: routeLabels.map((route) => DropdownMenuItem<String>(value: route, child: Text(route))).toList(), onChanged: (value) { if (value != null) setState(() => _route = value); }),
      const SizedBox(height: 10),
      Row(children: [Expanded(child: DropdownButtonFormField<String>(value: _year, decoration: _decoration('년도', Icons.calendar_today), items: const ['전체', '2026년', '2027년', '2028년'].map((year) => DropdownMenuItem<String>(value: year, child: Text(year))).toList(), onChanged: (value) { if (value != null) setState(() => _year = value); })), const SizedBox(width: 10), Expanded(child: DropdownButtonFormField<String>(value: _voyage, decoration: _decoration('항차', Icons.confirmation_number_outlined), items: const ['전체', '01항차', '02항차', '03항차', '04항차', '05항차'].map((voyage) => DropdownMenuItem<String>(value: voyage, child: Text(voyage))).toList(), onChanged: (value) { if (value != null) setState(() => _voyage = value); }))]),
      const SizedBox(height: 10),
      ...
      if (_searched) ...[const SizedBox(height: 16), Text('화물 정보 (${results.length}건)', style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.primary)), const SizedBox(height: 8), if (results.isEmpty) const Text('검색 결과가 없습니다.', style: TextStyle(color: AppColors.textSecondary)), ...results.map((item) => Card(child: ListTile(selected: _selectedIds.contains(item['id']), onTap: () => _toggleSelection(item), leading: Checkbox(value: _selectedIds.contains(item['id']), onChanged: (_) => _toggleSelection(item)), title: Text(item['invoice'] ?? ''), subtitle: Text('${item['name']}  ·  ${item['phone']}\n${item['route']}  ·  ${item['year']}  ·  ${item['voyage']}'), isThreeLine: true)))],
      const SizedBox(height: 16),
      if (_selectedIds.isNotEmpty) ...[const Text('선택한 화물 정보 수정', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.primary)), const SizedBox(height: 8), TextField(controller: _invoiceController, decoration: _decoration('송장번호', Icons.receipt_long_outlined)), const SizedBox(height: 10), TextField(controller: _nameController, decoration: _decoration('이름/라오스 수령인', Icons.person_outline)), const SizedBox(height: 10), TextField(controller: _phoneController, keyboardType: TextInputType.phone, decoration: _decoration('연락처', Icons.phone_outlined)), const SizedBox(height: 10), TextField(controller: _noteController, maxLines: 3, decoration: _decoration('수정 내용 메모', Icons.edit_note_outlined)), const SizedBox(height: 14), Row(children: [Expanded(child: SizedBox(height: 48, child: ElevatedButton.icon(onPressed: _save, icon: Icon(_canSaveDirectly ? Icons.save : Icons.send), label: Text(_canSaveDirectly ? '화물 정보 저장' : '화물 정보 수정 요청'), style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white)))), const SizedBox(width: 10), Expanded(child: SizedBox(height: 48, child: OutlinedButton(onPressed: _cancelSelection, child: const Text('취소'))))])],
      if (_isAdmin) ...[const SizedBox(height: 22), OutlinedButton.icon(onPressed: _openManagementMenu, icon: const Icon(Icons.settings), label: const Text('일정·공지 관리'))],
    ]);
  }
}
