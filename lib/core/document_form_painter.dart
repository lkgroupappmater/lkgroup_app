...
class DocumentFormPainter {
  ...
  static void header(Canvas c, {required ui.Image logo, required String title,
    ...
    DocumentFormStyle.drawText(c, tr('구획(Zone)'),
    ...
    for (var i = 0; i < 3; i++) {
      for (var side = 0; side < 2; side++) {
        ...
        DocumentFormStyle.drawText(c, tr(pair.$1), label.deflate(6), 20,
          center: true, bold: true, lineHeight: 1.25);
        DocumentFormStyle.drawText(c, pair.$2.isEmpty ? '-' : (side == 0 ? tr(pair.$2) : pair.$2),
  ...
  static void notes(Canvas c, DocumentFormLayout layout,
      ...
      String delivery = '', Color? deliveryColor, AppLanguage language = AppLanguage.korean}) {
    ...
    DocumentFormStyle.drawText(c, DocumentLocalizations.text(language, 'Remark/비고'),
      Rect.fromLTWH(12, top + 8, DocumentFormStyle.remarkWidth, 32), 23, bold: true);
    DocumentFormStyle.drawText(c, DocumentLocalizations.text(language, 'Inland delivery/시내·지방 배송'),
  ...
  static void totals(Canvas c, DocumentFormLayout layout,
      ...
      bool accounting = false, AppLanguage language = AppLanguage.korean}) {
    ...
    for (var i = 0; i < adjustments.length; i++) {
      ...
      DocumentFormStyle.drawText(c, DocumentLocalizations.text(language, row.$1),
    ...
    DocumentFormStyle.drawText(c, DocumentLocalizations.text(language, label),
  ...
  static void footer(Canvas c, DocumentFormLayout layout,
    ...
    DocumentFormStyle.drawText(c, tr('엘케이(LK)무역'),
    ...
    DocumentFormStyle.drawText(c, tr('고객사 확인'),
      Rect.fromLTWH(w - signW + 12, top + 10, signW - 24, 28), 21, bold: true, top: true);
    final number = NumberFormat('#,##0.##', 'en_US');
    DocumentFormStyle.drawText(c,
      '${tr('적용 환율')} · 1 USD = ${number.format(kipRate)} LAK · 1 USD = ${number.format(thbRate)} THB · 1 USD = ${number.format(krwRate)} KRW',
