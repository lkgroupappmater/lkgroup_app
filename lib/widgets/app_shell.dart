...
class _AppShellState extends State<AppShell> {
  ...
  void _onLanguageChanged(AppLanguage language) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('${language.flag} ${language.label}')),
    );
  }

  void _openAccount() => setState(() => _currentIndex = 3);
  ...
  void _onLoggedIn(AppUser user) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('${user.name.isEmpty ? '회원' : user.name}님 로그인되었습니다.')),
    );
  }
  ...
  Widget build(BuildContext context) {
    final tabs = <Widget>[
      ...
      ShipmentSearchBody(
        ...
        onEditRequest: () => setState(() => _currentIndex = 4),
      ),
      QuoteRequestBody(
        language: _language,
      ...
      AccountBody(
        ...
        onOpenCargoManagement: _isLoggedIn
    ...
    final labels = <String>[
      AppStrings.get(_language, 'home'),
      AppStrings.get(_language, 'tracking'),
      AppStrings.get(_language, 'quote'),
      AppStrings.get(_language, 'account'),
      '화물 관리',
    ];
    ...
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: [
          ...
          BottomNavigationBarItem(icon: const Icon(Icons.person_outline), activeIcon: const Icon(Icons.person), label: labels[3]),
          if (_isLoggedIn)
            BottomNavigationBarItem(icon: const Icon(Icons.inventory_2_outlined), activeIcon: const Icon(Icons.inventory_2), label: labels[4]),
        ],
      ),
    );
  }
