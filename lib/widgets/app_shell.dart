...
class _AppShellState extends State<AppShell> {
  ...
  void _onLanguageChanged(AppLanguage value) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('${value.flag} ${value.label}')),
    );
  }
  ...
  void _onLoggedIn(AppUser user) {
    setState(() {
      _currentUser = user;
      _currentIndex = user.role.canManageCargo ? 3 : 0;
    });
    ScaffoldMessenger.of(context).showSnackBar(
      TextSnackBar('${user.name.isEmpty ? '회원' : user.name}님 로그인되었습니다.'),
    );
  }
